...
export async function createClient(input: CreateClientInput): Promise<Result<Client>> {
  ...
  try {
    ...
    return err(new ExternalApiError('asaas', `Erro ao criar cliente: ${(error as Error).message}`, undefined, error));
  }
}

export async function searchClients(query: string): Promise<Result<Client[]>> {
  ...
  try {
    ...
    return err(new ExternalApiError('asaas', `Erro ao buscar clientes: ${(error as Error).message}`, undefined, error));
  }
}

export async function getClientByAsaasId(asaasId: string): Promise<Result<Client>> {
  ...
  try {
    ...
    return err(new ExternalApiError('asaas', `Erro ao buscar cliente: ${(error as Error).message}`, undefined, error));
  }
}
