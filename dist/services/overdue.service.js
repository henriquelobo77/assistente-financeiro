...
export async function getOverdueReport() {
    ...
    try {
        ...
        const topCharges = charges
            ...
            .slice(0, 20);
        const enrichedCharges = await Promise.all(topCharges.map(async (charge) => {
            try {
        ...
        return err(new ExternalApiError('asaas', `Erro ao gerar relatorio: ${error.message}`, undefined, error));
    }
}
export async function getOverdueByClient(customerIdOrCpf) {
    ...
    try {
        ...
        const charges = response.data.map((raw) => ({
            id: raw.id,
        ...
        return err(new ExternalApiError('asaas', `Erro ao consultar inadimplencia: ${error.message}`, undefined, error));
    }
}
